...
class JoinClausePattern(Pattern):
    ...
    def match(self, context: MatchContext) -> MatchResult:
        ...
        if not (isinstance(current, Token) and 
                current.type == TokenType.KEYWORD and 
                current.value.upper() in join_type_keywords):
            return MatchResult(success=False)
        ...
        while not current_context.at_end():
            ...
            if isinstance(curr, Token) and curr.type == TokenType.KEYWORD:
                ...
                if keyword in join_type_keywords:
                    ...
                    break
            elif isinstance(curr, Token) and curr.type in (TokenType.WHITESPACE, TokenType.NEWLINE):
                matched_tokens.append(curr)
                current_context = current_context.advance(1)
            else:
                break
        ...
        while not current_context.at_end():
            ...
            if (isinstance(curr, Token) and 
                curr.type == TokenType.KEYWORD and 
                paren_depth == 0):
                keyword = curr.value.upper()
                ...
                if keyword == 'ON':
                    ...
                    while not current_context.at_end():
                        ...
                        if (isinstance(curr, Token) and 
                            ...
                            paren_depth == 0):
                            kw = curr.value.upper()
                            if kw in self.clause_end_keywords or kw in join_type_keywords:
                                break
                ...
                elif keyword in self.clause_end_keywords or keyword in join_type_keywords:
                    break
...
class PivotUnpivotPattern(Pattern):
    ...
    def match(self, context: MatchContext) -> MatchResult:
        ...
        if not (isinstance(current, Token) and 
                current.type == TokenType.KEYWORD and 
                current.value.upper() in ('PIVOT', 'UNPIVOT')):
            return MatchResult(success=False)
        ...
        while not current_context.at_end():
            curr = current_context.current()
            if isinstance(curr, Token) and curr.type in (TokenType.WHITESPACE, TokenType.NEWLINE):
                matched_tokens.append(curr)
                current_context = current_context.advance(1)
            else:
                break
