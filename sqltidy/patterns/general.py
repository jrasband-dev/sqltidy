            ...
            if isinstance(curr, Token) and curr.value == '(':
                ...
                while not current_context.peek(peek_idx) is None:
                    ...
                    if isinstance(peek, Token):
                        if peek.type == TokenType.KEYWORD and peek.value.upper() == 'AS':
                            is_column_list = True
                            break
                        elif peek.type == TokenType.KEYWORD and peek.value.upper() == 'SELECT':
                            is_column_list = False
                            break
                    peek_idx += 1
...
class WindowFunctionPattern(Pattern):
    ...
    def match(self, context: MatchContext) -> MatchResult:
        ...
        curr = current_context.current()
        if not (isinstance(curr, Token) and 
                curr.type == TokenType.KEYWORD and 
                curr.value.upper() == 'OVER'):
            return MatchResult(success=False)
