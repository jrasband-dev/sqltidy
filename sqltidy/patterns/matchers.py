...
class KeywordPattern(Pattern):
    ...
    def match(self, context: MatchContext) -> MatchResult:
        ...
        if (isinstance(current, Token) and 
            current.type == TokenType.KEYWORD and 
            current.value.upper() == self.keyword):
            return MatchResult(
                success=True,
                matched_tokens=[current],
                end_index=context.start_index + 1
...
class KeywordSetPattern(Pattern):
    ...
    def match(self, context: MatchContext) -> MatchResult:
        ...
        if (isinstance(current, Token) and 
            current.type == TokenType.KEYWORD and 
            current.value.upper() in self.keywords):
            return MatchResult(
                success=True,
                matched_tokens=[current],
                end_index=context.start_index + 1,
...
class UntilPattern(Pattern):
    ...
    def __init__(self, stop_pattern: Pattern, 
...
class BetweenPattern(Pattern):
    ...
    def __init__(self, start_pattern: Pattern, end_pattern: Pattern,
        ...
        Args:
            ...
            name: Optional name for this pattern
        """
        super().__init__(name=name or f"Between({start_pattern.name},{end_pattern.name})")
        ...
        while not current_context.at_end() and depth > 0:
            ...
            if end_result.success:
                ...
                if depth == 0:
                    ...
                    current_context = current_context.advance(len(end_result.matched_tokens))
                    break
...
class WhitespacePattern(Pattern):
    ...
    def match(self, context: MatchContext) -> MatchResult:
        ...
        if (isinstance(current, Token) and 
            current.type in (TokenType.WHITESPACE, TokenType.NEWLINE)):
            return MatchResult(
                success=True,
                matched_tokens=[current],
                end_index=context.start_index + 1
