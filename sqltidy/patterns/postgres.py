...
class ReturningClausePattern(Pattern):
    ...
    def __init__(self, name: Optional[str] = None):
        super().__init__(
            name=name or "ReturningClause",
            supported_dialects=['postgresql', 'oracle']
        )
    ...
    def match(self, context: MatchContext) -> MatchResult:
        ...
        clause_end_keywords = {
            'INTO', 'FROM', 'WHERE', 'GROUP', 'HAVING', 'ORDER',
            'LIMIT', 'OFFSET', 'FETCH', 'INSERT', 'UPDATE', 'DELETE'
        }
        ...
        while not current_context.at_end():
            ...
            if isinstance(curr, Token):
                if curr.type in (TokenType.WHITESPACE, TokenType.NEWLINE):
                    matched_tokens.append(curr)
                    current_context = current_context.advance(1)
                elif curr.type == TokenType.KEYWORD and curr.value.upper() == 'CONFLICT':
                    matched_tokens.append(curr)
                    current_context = current_context.advance(1)
                    break
                else:
        ...
        while not current_context.at_end():
            ...
            if isinstance(curr, Token):
                if curr.value == '(':
                    ...
                    paren_depth -= 1
                elif curr.type == TokenType.KEYWORD and curr.value.upper() == 'DO' and paren_depth == 0:
                    found_do = True
                    matched_tokens.append(curr)
                    current_context = current_context.advance(1)
                    break
            ...
            if isinstance(curr, Token):
                if curr.type in (TokenType.WHITESPACE, TokenType.NEWLINE):
                    ...
                    if keyword == 'NOTHING':
                        ...
                        clause_end_keywords = {
                            'WHERE', 'RETURNING', 'INSERT', 'UPDATE', 'DELETE', 'SELECT'
                        }
