    ...
    def _process_tokens(self, tokens: List[Union[Token, TokenGroup]]) -> List[Union[Token, TokenGroup]]:
        ...
        while i < len(tokens):
            ...
            if isinstance(token, TokenGroup):
                ...
                i += 1
                continue
            if isinstance(token, Token) and token.type == TokenType.KEYWORD and token.value.upper() == "TOP":
                next_token = tokens[i + 1] if i + 1 < len(tokens) else None
                if next_token and isinstance(next_token, Token) and next_token.type == TokenType.PUNCTUATION and next_token.value == "(":
                    result.append(token)
