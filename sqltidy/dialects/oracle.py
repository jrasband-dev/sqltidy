...
class OracleDialect(SQLDialect):
    ...
    def keywords(self) -> Set[str]:
        ...
        return {kw.lower() for kw in {
            # Core SQL keywords
            'SELECT', 'FROM', 'WHERE', 'INSERT', 'UPDATE', 'DELETE',
            'CREATE', 'ALTER', 'DROP', 'TRUNCATE', 'RENAME',
            'TABLE', 'VIEW', 'INDEX', 'SEQUENCE', 'TRIGGER', 'PROCEDURE', 'FUNCTION',
            'PACKAGE', 'BODY', 'TYPE', 'SYNONYM', 'DATABASE', 'SCHEMA',
            'TABLESPACE', 'USER', 'ROLE', 'PROFILE',
            ...
            'IF', 'ELSIF', 'ELSEIF', 'ENDIF',
            
            # Logical operators
            'AND', 'OR', 'NOT', 'IN', 'EXISTS', 'BETWEEN', 'LIKE',
            'IS', 'NULL', 'ANY', 'SOME', 'ALL',
            
            # Constraints
            'CONSTRAINT', 'PRIMARY', 'KEY', 'FOREIGN', 'REFERENCES',
            'UNIQUE', 'CHECK', 'DEFAULT', 'NOT',
            'DEFERRABLE', 'INITIALLY', 'DEFERRED', 'IMMEDIATE',
            
            # Oracle-specific hierarchical queries
            'CONNECT', 'START', 'WITH', 'PRIOR', 'LEVEL',
            'NOCYCLE', 'SYS_CONNECT_BY_PATH',
            
            # Oracle-specific keywords
            'DUAL', 'ROWNUM', 'ROWID', 'UROWID', 'NEXTVAL', 'CURRVAL',
            'SYSDATE', 'SYSTIMESTAMP', 'CURRENT_DATE', 'CURRENT_TIMESTAMP',
            
            # PL/SQL block structure
            'DECLARE', 'BEGIN', 'END', 'EXCEPTION', 'RAISE',
            ...
            # Data types
            'VARCHAR2', 'NVARCHAR2', 'CHAR', 'NCHAR', 'NUMBER',
            'INTEGER', 'INT', 'SMALLINT', 'FLOAT', 'REAL', 'DOUBLE', 'PRECISION',
            'DATE', 'TIMESTAMP', 'INTERVAL', 'DAY', 'MONTH', 'YEAR', 'TO',
            'CLOB', 'NCLOB', 'BLOB', 'BFILE', 'RAW', 'LONG',
            'BINARY_FLOAT', 'BINARY_DOUBLE',
            'BOOLEAN', 'PLS_INTEGER', 'BINARY_INTEGER',
            'ROWTYPE', 'RECORD', 'REF', 'VARRAY', 'NESTED',
            
            # Table/Index options
            'COMPRESS', 'NOCOMPRESS', 'LOGGING', 'NOLOGGING',
            'CACHE', 'NOCACHE', 'PARALLEL', 'NOPARALLEL',
            'PCTFREE', 'PCTUSED', 'INITRANS', 'MAXTRANS',
            'STORAGE', 'INITIAL', 'NEXT', 'MINEXTENTS', 'MAXEXTENTS',
            
            # Partitioning
            'PARTITION', 'SUBPARTITION', 'RANGE', 'LIST', 'HASH',
            'PARTITIONS', 'SUBPARTITIONS', 'STORE', 'OVERFLOW',
            
            # Privileges
            'GRANT', 'REVOKE', 'PRIVILEGES', 'ADMIN', 'OPTION',
            'CASCADE', 'RESTRICT',
            
            # Transaction control
            'COMMIT', 'ROLLBACK', 'SAVEPOINT', 'WORK',
            'READ', 'WRITE', 'ONLY', 'SERIALIZABLE', 'ISOLATION',
            
            # Lock
            'LOCK', 'SHARE', 'EXCLUSIVE', 'MODE', 'NOWAIT', 'WAIT',
            ...
            'CONCAT', 'LPAD', 'RPAD', 'ASCII', 'CHR',
            
            # Conversion functions
            'TO_CHAR', 'TO_DATE', 'TO_NUMBER', 'TO_TIMESTAMP',
            'TO_CLOB', 'TO_BLOB', 'CAST', 'CONVERT',
            ...
            # Other functions
            'GREATEST', 'LEAST', 'DUMP', 'VSIZE', 'USER', 'UID',
            
            # Metadata/System
            'COMMENT', 'ANALYZE', 'VALIDATE', 'STRUCTURE',
            'COMPUTE', 'STATISTICS', 'ESTIMATE',
            
            # Materialized views
            'MATERIALIZED', 'REFRESH', 'FAST', 'COMPLETE', 'FORCE',
            'ON', 'DEMAND', 'COMMIT', 'BUILD', 'DEFERRED',
            
            # Flashback
            'FLASHBACK', 'VERSIONS', 'SCN', 'TIMESTAMP', 'AS OF',
            
            # Oracle Text
            'CONTAINS', 'CATSEARCH', 'MATCHES',
            
            # XML
            'XMLTYPE', 'XMLELEMENT', 'XMLATTRIBUTES', 'XMLFOREST',
            'XMLAGG', 'XMLPARSE', 'XMLSERIALIZE',
            
            # Collections
            'TABLE', 'CAST', 'MULTISET', 'MEMBER', 'SUBMULTISET',
            'THE', 'CARDINALITY',
            
            # Advanced features
            'PIVOT', 'UNPIVOT', 'MODEL', 'DIMENSION', 'MEASURES',
            'RULES', 'AUTOMATIC', 'SEQUENTIAL',
            
            # Hints (common ones)
            'FULL', 'INDEX', 'PARALLEL', 'USE_NL', 'USE_HASH',
            'USE_MERGE', 'APPEND', 'CACHE', 'NOCACHE',
            ...
            'ACCESSIBLE', 'IDENTIFIED', 'PASSWORD', 'EXPIRE',
            'ACCOUNT', 'UNLOCK', 'TEMPORARY', 'GLOBAL',
            'PRIVATE', 'PUBLIC', 'AUTHID', 'CURRENT_USER', 'DEFINER',
    ...
    def functions(self) -> Set[str]:
        ...
        return {fn.lower() for fn in {
            # Aggregate functions
            'COUNT', 'SUM', 'AVG', 'MIN', 'MAX',
            'STDDEV', 'VARIANCE', 'MEDIAN',
            'LISTAGG', 'GROUPING', 'GROUPING_ID',
            ...
            'CAST', 'CONVERT', 'HEXTORAW', 'RAWTOHEX',
            ...
            'ADD_MONTHS', 'MONTHS_BETWEEN', 'NEXT_DAY', 'LAST_DAY',
            'TRUNC', 'ROUND', 'EXTRACT',
            'NEW_TIME', 'TZ_OFFSET', 'FROM_TZ', 'TO_TIMESTAMP_TZ',
            'NUMTODSINTERVAL', 'NUMTOYMINTERVAL',
            
            # Numeric functions
            'ABS', 'CEIL', 'FLOOR', 'MOD', 'POWER', 'SQRT',
            'SIGN', 'EXP', 'LN', 'LOG', 'ROUND', 'TRUNC',
            ...
            'USER', 'UID', 'USERENV', 'SYS_CONTEXT', 'SYS_GUID',
            'DUMP', 'VSIZE',
            ...
            'XMLPARSE', 'XMLSERIALIZE', 'XMLQUERY', 'XMLTABLE',
            'EXTRACTVALUE', 'XMLCAST', 'XMLCOLATTVAL', 'XMLCONCAT',
            ...
            # Other functions
            'SOUNDEX', 'BITAND', 'BITOR', 'BITXOR', 'BITNOT',
